...
Someone asked for an HTML guy on the Brotato Discord to help out in building a Twitch overlay mod, and I'm an HTML kind of guy, so I said I'll do it!

## How?

Very good question. I never build something with twitch APIs before, so I struggled for some time before I managed to set everything up.

Luckily I had help from pasha ho had asked for the HTML guy on Discord. He has some experience in that field for his **[TwitchSlaysSpire](https://www.twitch.tv/twitchslaysspire)** channel.

## Big Picture

At the end we have 3 main components:
...
I will skip over the authentication part, most of that handled pasha so I don’t have all the details.
...
All the user can see from the Mod are these 2 Buttons on the main menu. One to login and one to toggle the data collection.  
...
- The `SendTimer` calls the `“Sender”` every 3 seconds, the Sender decides to create an `Update Batch` or an `Catch Up Batch` and sends it to the Twitch Pubsub Endpoint.
...
Lets take a peak at what the overlay actually looks like

![twitch overlay show case](./twitch_overlay.webm)

<details>
	<summary>
	👇 Here is one example batch of data that is send to the Overlay 👇
	</summary>
...
Currently this are all the actions needed:
...
But there exist a lot of cool mods for Brotato and most of them have there one unique Icons.
The Blobfish is nice but wasn’t to my full satisfaction. That’s why the Image Upload block exists up there in the overview.
I decided to squeeze this 96x96 images over the pubsub wire via base64 string.  
...
- If not create the base64 string from the `Image` Resource
  ...
  ```
- Split that string into chunks, I decided to go with 3500 chars per chunk, leaving a bit space to the 4 kb limit, just to be save.
- Add that image data to the queue, so it is send by the “Sender".
  ![screenshot of godot dictionary editor showing structure of the image update queue dictionry. Each image is stored by item_id key as dictionary in that dictionary is item_id, base_64_chunks and base_64_chunk_count.](./upload_queue_image.png)
...
One issue some one has to solve if some one only sends part of a state to all clients is the fact that all clients that join late to the party missed out on the first batches of data. To solve this all the of the orange part exists up there in the overview.
...
Each time a chunk of data is send by the Sender it is stored in one of the `catch_up_stores`.   
Each of the stores are based on the `item_id`, my first go was one store array with all actions. But that slowed the catch up down to a crawl, adding and removing weapons just to add new ones again is hardly optimized. 😄   
With the current solution each batch of `catch_up` data is valuable, no duplicates, no adding and removing.
...
Here is the final product again. Nothing to crazy just 3 panels, tooltips and some buttons.   
...
I had to figure out how to make things work with Svelte. Luckily someone else had build a small extension in Svelte before, so I had something to get _inspiration_ from.

[Deep Rock Galactic Loadouts Twitch Extension](https://github.com/ajzbc/drg-loadout-twitch-extension)

At the end you want to have a html file as entry point or depending on your type of extension multiple. With the `input` option it’s possible to define thous in the Vite config.
...
I switched to SvelteKit for a bit, just to allow me to spin up an endpoint for sending test data so I don’t have to use pubsub all the time during development. At the end I had to switch back to plain Svelte because I didn’t manage to get an export working without violating the content-security-policies. Because SvelteKit initializes with inline JS and that is a no go for Twitch Extensions. If you know how to get SvelteKit to not do this please let me know 😶.

### Testing on Twitch

I don’t know why but I was surprised to learn that I have to run an actual live stream even if I use the “local test” available on the extension dashboard. I think there is an option to do a non public live stream but I didn’t bother, just know you will have to setup a stream to run a proper test for your extension if it is an overlay. With the livestream running and the extension activated I found some issues. You can’t put any interactions in the bottom 20-30px because of the video player controls and the open chat window overlaps the extension iframe for some reason a bit so a additional 20-30pxs are lost on the right. Nothing that can’t be fixed with a bit more padding. Also I highly recommend building and uploading the extension during the development a couple of times just to be sure everything works as expected, that is how I stumbled upon the content-security vs. SvelteKit issue.

### How things work on the Frontend

![Diagram of the frontend structure. Showing data_handler.js with data handler methods for each action. stores.js with svelte stores for general app data, game_data and some derived stores for sorting items. Then a seperate bouble for static files for all vanilla icons and translation json file. Then the basic components with Video OVerlay on the top then Container, Container_Items and Container_Stats underneath to the left. On the Right the Tooltip_Items component.](./frontend_components.png)

I will not go into great detail, because most of the stuff is very basic.   

### Receiving Data / Actions

To receive the pubsub messages from the backend the Twitch JS helper is used. That is directly loaded from twitch in the entry html file.
...
It also keeps the received `game_data` in memory and stores all `handled_actions` to check and possibly discard already received actions. At the end it returns the new `game_data` that is stored in the `$game_data` [store](https://svelte.dev/docs/svelte-store). From this store all components have access to the latest `game_data` .

### BB Code

Something a bit non standard is the parsing of the BB Code used in the item and weapon effect / stat text. There are a couple of BB Code parser out there I decided on [BBob](https://github.com/JiLiZART/BBob) thankfully with Svelte it’s possible to just use the HTML renderer. It can parse all the BB Code used in Brotato. So we can just grab the full text string send it through the parser and can output the raw html with `[@html](https://svelte.dev/docs/special-tags#html)`. Some strings have references to stat icons so I `replaceAll()` references to `'res://items/stats’` with `stat_icons` to point them to the stat icons in the `static` directory. With that it was quiet easy to get the look and feal of the in-game ui for the tooltips.

![Video of the working tooltip](./tooltip.mp4)

### Animations

Nothing crazy here so I just add some quick bullet points about the used animations.
...
- Build in transitions for the settings panel and tooltip.
